...
if [ "$RESET_OUT" != "ok" ]; then
    # При первом выкате docroot сайта ещё указывает на старый каталог, и запрос
    # уходит мимо только что собранного релиза — это не сбой, а ожидаемое
    # состояние до переключения docroot в панели.
    if [ "${OPCACHE_OPTIONAL:-0}" = "1" ]; then
        echo "OPcache не сброшен (ответ: $RESET_OUT) — ожидаемо, пока docroot не переключён на current" >&2
    else
