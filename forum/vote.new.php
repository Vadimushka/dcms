...
VALUES ('$user->id', '$theme[id]', '$vote', " . implode(', ', $v) . ")");
            ...
            if (!$id_vote = mysql_insert_id())$doc->err(__('При создании голосования возникла ошибка'));
            else {
                if (isset($_GET['return']))header('Refresh: 1; url=' . $_GET['return']);
                else header('Refresh: 1; url=theme.php?id=' . $theme['id']);
...
$elements = array();
$elements[] = array('type' => 'textarea', 'title' => __('Вопрос'), 'br' => 1, 'info' => array('name' => 'vote'));

for ($i = 1;$i <= 10;$i++)
$elements[] = array('type' => 'input_text', 'title' => __('Ответ №') . $i, 'br' => 1, 'info' => array('name' => "v$i"));

$elements[] = array('type' => 'submit', 'br' => 0, 'info' => array('value' => __('Создать голосование'))); // кнопка
